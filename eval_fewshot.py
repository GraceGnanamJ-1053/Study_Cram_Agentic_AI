...
#--- NEW IMPORTS : Register Custom MoR Architectures ---
...
from model.recursive_model.modeling_llama import *
#-------------------------------------------------------
...
def evaluate_model(cfg: DictConfig, eval_logger=None, writer=None, step=None):
    #--------------------------------------------------------
    # import sys;print(f"DEBUG: cfg.model_name_or_path is{cfg.model_name_or_path}");sys.exit(1)
    # 1. Define the correct, absolute path to your checkpoint folder. 
    # We construct it using PROJECT_ROOT and the known folder name (cfg.name). 
    correct_local_checkpoint_folder = os.path.join( PROJECT_ROOT, "checkpoints", cfg.name ) 
    # 2. OVERWRITE the corrupted path variable in the config object itself. 
    cfg.model_name_or_path = correct_local_checkpoint_folder 
    #--------------------------------------------------------
    if cfg.model in ["hf-auto", "hf", "huggingface"]:
        ...
        if cfg.relaxation.enable:
            ...
            if "mor" in cfg and cfg.mor.get("enable"):
                # added the 1 line:
                local_checkpoint_path = cfg.model_name_or_path
        ...
        # ----------------- FINAL PATH OVERRIDE INJECTION ----------------- 
        # Construct the correct local path based on the known location and naming convention. 
        # This bypasses the complex, buggy path resolution logic. 
        correct_local_path = os.path.join(PROJECT_ROOT, "checkpoints", cfg.name) 
        # Overwrite the path variable used by AutoConfig with the correct local path. 
        # This is required because the current path resolves to 'results/pretrain/...' 
        cfg.model_name_or_path = correct_local_path 
        # ----------------- INJECTION END ----------------- 
